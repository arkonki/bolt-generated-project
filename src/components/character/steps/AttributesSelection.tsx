...
const rollAttribute = (): number => {
  ...
  return rolls.slice(0, 3).reduce((sum, roll) => sum + roll, 0); // Sum the highest 3 rolls
};
...
export function AttributesSelection() {
  ...
  const handleRollAll = () => {
    ...
    // Calculate final values with age modifiers
    ...
    // Update attributes with base chances
    ...
    updateCharacter({ attributes: finalAttributes });
    setIsRolling(false);
  };
  ...
  const handleManualEntry = (attr: string, value: string) => {
    ...
    // Update character with final values including age modifiers
    updateCharacter({
      attributes: Object.entries(newAttributes).reduce(
        (acc, [key, { value }]) => ({
          ...acc,
